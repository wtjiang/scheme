...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if formals.__len__() != vals.__len__():
            raise SchemeError('Number of parameters not equal to values!')
        else:
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        operandz = operands.map(lambda oper: scheme_eval(oper, env))
        return scheme_apply(self ,operandz, env)
        # END PROBLEM 5
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        value = do_lambda_form(expressions, env)
        env.define(target, value)
        return target
...
def do_let_form(expressions, env):
    """Evaluate a let form."""
    #return expressions.first.first.second.first
    check_form(expressions, 2)
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        if type(bindings.first.first) == int:
            raise SchemeError("wrong formal type")
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            # BEGIN Extra Credit
            oper = scheme_optimized_eval(first, env)
            check_procedure(oper)
            result = oper.eval_call(rest, env)
            # END Extra Credit
    return result
